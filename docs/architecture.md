...
## 서비스 책임
- authz-service는 Gateway가 관리자 경로(`/admin/**`, `/v1/admin/**`) 요청을 처리하기 전에 최종 인가 판정을 제공하는 내부 서비스입니다.
- 에디터 서버(editor-service) 관리자 API 보호를 위한 capability 진실을 소유합니다.
- user-service의 인증 역할이나 프로필 공개 정책은 직접 신뢰하지 않고, Authz DB의 권한 역할만 인가 근거로 사용합니다.
- 실제 기능 집행은 소비자 서비스가 최종 강제하고, authz-service는 허용/거부 판정을 반환합니다.

## Repository Context
- 구현 레포: `https://github.com/jho951/authz-service`
- 로컬 개발 경로: `/Users/jhons/Downloads/BE/services/authz-service`
- Gateway 연동 대상: `/Users/jhons/Downloads/BE/services/gateway-service`
- Editor(editor-service) 연동 대상: `/Users/jhons/Downloads/BE/services/editor-service`
...
- Spring Data JPA
- Redis
- MySQL runtime datasource + H2 test 전용
- Gradle 멀티모듈

## 모듈 구조
- `common`: authz-service 내부 공통 기반 모듈입니다.
...
2. Gateway가 authz-service의 `POST /permissions/internal/admin/verify`를 호출합니다.
3. authz-service는 내부 caller proof, 필수 헤더, 원본 method/path 형식을 검증합니다.
...
- auth-service와 맞춘 `app`, `common` 멀티모듈 구조를 사용합니다.
